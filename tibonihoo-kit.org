...
(custom-set-faces
 ...
 '(highlight ((t (:background "brightblack" :foreground "yellow")))))
#+end_src
...
(use-package idle-highlight-in-visible-buffers-mode
  :config 
  (add-hook 'text-mode-hook 'idle-highlight-in-visible-buffers-mode)
  )
#+end_src
...
#+begin_src emacs-lisp
  (use-package wgrep)
  (grep-apply-setting 'grep-command "grep --color=always -nH --null -r ")
#+end_src  
...
matching proved helpful in coping with the typo I make when in a
hurry. Also with some optmisation to the matching algorithm from
flx-ido.
#+begin_src emacs-lisp
